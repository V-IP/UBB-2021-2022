...
void testAdd() {
	Repository testRepo;
	WishList testWishList{ testRepo };
	Validator testValid;
	Service testSrv{ testRepo,testWishList, testValid };
...
void testFileRepo() {
	...
	ofs << "1,Name1,Type1,10\n";
	ofs << "2,Name2,Type2,20\n";
	ofs << "3,Name3,Type3,30\n";
	ofs << "4,Name4,Type4,40\n";
	ofs.close();
	try {
	...
	Tenant found = testFileRepo.find(2);
	assert(found.getOwner() == "Name2");
	assert(found.getType() == "Type2");
	assert(found.getSurface() == 20);

	Tenant t5{ 5,"Name5","Type5",50 };

	testFileRepo.add(t5);
	assert(testFileRepo.getAll().size() == 5);
}